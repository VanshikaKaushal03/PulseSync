...
import { Filter, Plus, Save, Play, Code, CheckCircle, Database, Trash2, ChevronDown } from 'lucide-react';

// Maps each database source to its known filterable fields and collections
...
function saveSubsToStorage(subs: Subscription[]) {
  try {
...
function loadActiveId(subs: Subscription[]): string {
  try {
    ...
    if (id && subs.find(s => s.id === id)) return id;
  } catch {}
  return subs[0]?.id || '';
}
...
function buildMongoFilter(rules: Rule[]) {
  ...
  for (const rule of rules) {
    ...
    if (rule.operator === 'equals') {
      filterObj[rule.field] = val;
    } else if (rule.operator === 'not_equals') {
      filterObj[rule.field] = { $ne: val };
    } else if (rule.operator === 'contains') {
      filterObj[rule.field] = { $regex: val, $options: 'i' };
    } else if (rule.operator === 'greater_than') {
      filterObj[rule.field] = { $gt: parseFloat(val) || val };
    }
  }
  return filterObj;
}

export function SmartSubscriptions() {
  const [sources, setSources] = useState<any[]>([]);
  const [subscriptions, setSubscriptions] = useState<Subscription[]>(() => loadSubs());
  const [activeSubId, setActiveSubId] = useState<string>(() => {
    const subs = loadSubs();
    return loadActiveId(subs);
  });
  ...
  useEffect(() => {
    saveSubsToStorage(subscriptions);
  }, [subscriptions]);

  // Persist active sub ID whenever it changes
  useEffect(() => {
    try { localStorage.setItem(LS_ACTIVE_KEY, activeSubId); } catch {}
  }, [activeSubId]);
  ...
  const updateRule = (idx: number, patch: Partial<Rule>) => {
    const newRules = activeSub.rules.map((r, i) => i === idx ? { ...r, ...patch } : r);
    updateActiveSub({ rules: newRules });
  };
  ...
  const deleteSubscription = (id: string) => {
    setSubscriptions(prev => {
      ...
      const result = remaining.length ? remaining : DEFAULT_SUBS;
      saveSubsToStorage(result);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        db_name: activeSub.database,
        collection: activeSub.collection,
      ...
      console.error(e);
      setPreviewData({ error: e.message || 'Failed to connect to stream' });
    } finally {
  ...
  // Compiled filter uses real MongoDB operator names in the display too
  const compiledFilter = activeSub
    ? buildMongoFilter(activeSub.rules)
    : {};
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Select a database and build JSON filters to route specific events to your clients.</p>
        </div>
        <button
          onClick={handleSave}
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={e => {
                        ...
                        const firstColl = DB_SCHEMAS[db]?.collections[0] || '';
                        updateActiveSub({ database: db, collection: firstColl, rules: [{ field: '', operator: 'equals', value: '' }] });
                        setPreviewData(null);
                      }}
                  ...
                  {activeSub.rules.map((rule, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-gray-950 p-3 rounded-lg border border-gray-800">
                      ...
                      <input
                        ...
                        // Trim on blur so copy-paste whitespace is cleaned automatically
